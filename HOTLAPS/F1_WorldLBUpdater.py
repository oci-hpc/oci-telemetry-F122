    ...
    if fastestlaps is not None:
        ...
        if len(fastestlaps) == 1:
            ...
            try:
                ...
                print("--- No response from Red Bull server side. Call them.")
            ...
            if response is not None:
                ...
                elif "name" in response.text:
                    #print(response.text['name'])
                    if "lapalreadyexists" in response.text.lower():
                        print("    Success! Lap already uploaded.")

                        print("        Deleting lap: ", item[2], item[6])
                        delete_hotlap("hotlaps", item[2], item[6])
            else:
                print("No response from Red Bull server, will try to send again.")
                print(" ")
