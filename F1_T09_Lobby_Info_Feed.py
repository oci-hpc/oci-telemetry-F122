# Data provider for the F1_T09_Lobby_Info implementation
# Receives data from F1_Splitter via UDP Port 20787 and splits it and sends data to an specified UDP port for the OpenMCT server.
...
UDP_PORT_0 = 20787   #chosen port to Final Classification feed

UDP_IP = "127.0.0.1" #standard ip udp (localhost)
UDP_PORT =59022   #chosen port to OpenMCT (same as in telemetry server object)
...
while True:
    ...
    try:
        ...
        print("User stopped: F1_T09_Lobby_Info.")
        exit()
    except:
        print("Timeout: F1_T09_Lobby_Info waiting for more data from port:", UDP_PORT_0) 
    ...
    if message is not None:
        ...
        if message[5:6] == b'\x09':
            ...
            # Bytes
            byteCode = '<B' # accounts for num players
            for x in range(22):
            ...
            nameList = []
            for x in range(22):
                nameList.append(str(unpackedList[(x * 6) + position + 1].decode('utf8', 'strict').replace('\x00', '')))
            for x in range(22):
                unpackedList[(x * 6) + position + 1] = nameList[x]
            unpacked = tuple(unpackedList)


            # Full message send
            #MESSAGE = "{},{},{},{}".format(header + unpacked)
            MESSAGE = "{}".format(json.dumps(["packet_09", header, unpacked, timeStamp]));
            ...
            count = count + 1
            if count % 1 == 0:
                print("    Packet 09 messages sent: " + str(count))