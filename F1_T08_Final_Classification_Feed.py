# Data provider for the F1_T08_Final_Classification implementation
# Receives data from F1_Splitter via UDP Port 20785 and splits it and sends data to an specified UDP port for the OpenMCT server.
...
UDP_PORT_0 = 20786   #chosen port to Final Classification feed

UDP_IP = "127.0.0.1" #standard ip udp (localhost)
UDP_PORT =58022   #chosen port to OpenMCT (same as in telemetry server object)
...
while True:
    ...
    try:
        ...
        print("User stopped: F1_T08_Final_Classification.")
        exit()
    except:
        print("Timeout: F1_T08_Final_Classification waiting for more data from port:", UDP_PORT_0) 
    ...
    if message is not None:
        ...
        if message[5:6] == b'\x08':
            #b'\x08' = 8 Final Classification
            ...
            MESSAGE = "{}".format(json.dumps(["packet_08", header, unpacked, timeStamp]));
            ...
            count = count + 1
            if count % 1 == 0:
                print("    Packet 08 messages sent: " + str(count))