# Data provider for the F1_T02_Lap_Data implementation
# Receives data from F1_Splitter via UDP Port 20780 and splits it and sends data to an specified UDP port for the OpenMCT server.
...
UDP_PORT_0 = 20780   #chosen port to Lap Data feed

UDP_IP = "127.0.0.1" #standard ip udp (localhost)
UDP_PORT =52022   #chosen port to OpenMCT (same as in telemetry server object)
...
while True:
    ...
    try:
        ...
        print("User stopped: F1_T02_Lap_Data.")
        exit()
    except:
        print("Timeout: F1_T02_Lap_Data waiting for more data from port:", UDP_PORT_0) 
    ...
    if message is not None:
        ...
        if message[5:6] == b'\x02':
            ...
            unpacked = struct.unpack(byteCode, message[24:972])
            ...
            MESSAGE = "{}".format(json.dumps(["packet_02", header, unpacked, timeStamp]));
            ...
            count = count + 1
            if count % 1000 == 0:
                print("    Packet 02 messages sent: " + str(count))
